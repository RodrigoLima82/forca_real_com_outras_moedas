...
# ATENÇÃO: esse script é bastante pesado e pode levar até 1 hora para finalizar a execução
...
# Ajustando os dados
# Quando trabalhando com timeseries é importante garantir que todos os dados estejam na mesma
# "janela" (window) de tempo
...
# Perceba que temos 2 loops aninhados
# Essa operação é demorada e pode levar tempo dependendo do seu hardware
for(j in 1:38){
...
for(j in 1:38){
  ...
  for(d in 101:(nrDays-1)){
    ...
    # Calcula as correlação para o período
    model.data <- data.frame(response, predict)
...
# Mais uma vez perceba que temos um loop dentro de outro
# Isso serve apenas para agilizar e automatizar o processo
